...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
float pid_p_gain_roll = p_gain_roll;               //Gain setting for the roll P-controller
float pid_i_gain_roll = i_gain_roll;              //Gain setting for the roll I-controller
float pid_d_gain_roll = d_gain_roll;              //Gain setting for the roll D-controller
...
float pid_p_gain_yaw = p_gain_yaw;                //Gain setting for the pitch P-controller. //4.0
float pid_i_gain_yaw = i_gain_yaw;               //Gain setting for the pitch I-controller. //0.02
float pid_d_gain_yaw = d_gain_yaw;                //Gain setting for the pitch D-controller.
int pid_max_yaw = 400;                     //Maximum output of the PID-controller (+/-)

boolean auto_level = AUTO_LEVEL_MODE;                 //Auto level on (true) or off (false)
...
// function for getting from transmit (via timers controller)
ISR(PCINT0_vect){
  current_time = micros();
  //Channel 1=========================================
...
void EEA_drones::main_loop() {
  ...
  if (start == 0) {
    add_function_stopping_motor_mode();
  }
  if (start == 2) {
    add_function_starting_motor_mode();
  }
