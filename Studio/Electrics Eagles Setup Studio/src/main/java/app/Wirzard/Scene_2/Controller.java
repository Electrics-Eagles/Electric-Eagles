package app.Wirzard.Scene_2;

import app.API.CloneCoreFromGitHub.CloneRepo;
...
import static app.API.Generate_Config.Config.ConifgGenerator.generate_config_file;
...
public class Controller {
    ...
    void next(MouseEvent event) throws IOException {
       if(app.API.Varibles_Java.Variables.clone_flag) {
           File[] FXML = ScanFXML.ScanFXML("src\\main\\resources\\Wirzard.FXML\\Coding");
           FXML_Loader.OPEN_NEW_SCENE_NO_Style_SET(main_wirzard_windows, FXML[0].getAbsolutePath());
           Logger.INFO("Run an app.Wirzard Step 2", "log.txt");
   ...
   void add_static_varibles_to_combox() {
    ...
    void initialize() throws IOException {
        app.API.Varibles_Java.Variables.clone_flag=false;
        core_lib_link=found_data_in_config("CORE_LINK:{", "CORE_LINK];").get(0).get(0);
        new Thread(new CloneRepo()).start();
