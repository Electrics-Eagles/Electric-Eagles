...
SimpleKalmanFilter ultrasonics_kalman(1, 1, 0.01);
//COde by alex5250
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void detect_object() {
  if (ULTASONICS_SAFETY == 1) {
    ...
    if (ultrasonics_input_data < 30 ) {
      ...
      if (compensate > 200 && compensate < 1000)
        if (receiver_input_channel_2 + compensate > 1800) {
          receiver_input_channel_2 = 1800;
        }
        else {
          receiver_input_channel_2 = receiver_input_channel_2 + compensate;
        }
    }
...
void setup() {
  ...
  // Code by alex5250
  Serial.println("Set up BARO_STABILITY");
  ...
  if (DEBUG_MSG == 1) {
    ...
    Serial.println("Are you still want run a drone? Check a data.")
...
void loop() {
  ...
  float p = getPressure();
  float altitude = pressure.altitude(p, baseline);
  float altitude_now = pressureKalmanFilter.updateEstimate(altitude);
  unsigned long alt_time_two = millis();
  if (alt_time_two - alt_time_one > interval && BARO_STABILITY == 1) {
    // save the last time you blinked the LED
    alt_time_one = alt_time_two;
    if (altitude_now - altitude_before > 0 && altitude_now - altitude_before > 3 && altitude_before > 0 )
    {
      float correction = map(altitude_now - altitude_before, 0.1, 2.9, 300, 700);
      if (throttle + correction > 1800) {
        throttle = 1800;
        altitude_now = altitude_before;
      } else {
      ...
      if (altitude_now - altitude_before < 0 && altitude_now - altitude_before < -3 && altitude_before > 0 )
      {
        float correction = map(altitude_now - altitude_before, -2.9, -0.1, 700, 300);
        if (throttle + correction > 1800) {
          throttle = 1800;
          altitude_now = altitude_before;
        } else {
  ...
  if (start == 2) {                                                         //The motors are started.
    ...
    if (esc_1 < 1200) esc_1 = 1300;                                         //Keep the motors running.
    if (esc_2 < 1200) esc_2 = 1300;                                         //Keep the motors running.
    if (esc_3 < 1200) esc_3 = 1300;                                         //Keep the motors running.
    if (esc_4 < 1200) esc_4 = 1300;                                         //Keep the motors running.
    ...
    esc_1 = 1100;                                                           //If start is not 2 keep a 1000us pulse for ess-1.
    esc_2 = 1100;                                                           //If start is not 2 keep a 1000us pulse for ess-2.
    esc_3 = 1100;                                                           //If start is not 2 keep a 1000us pulse for ess-3.
    esc_4 = 1100;                                                           //If start is not 2 keep a 1000us pulse for ess-4.
  }
...
int convert_receiver_channel(byte function) {
  ...
  if (actual < center) {                                                       //The actual receiver value is lower than the center value
    ...
    if (reverse == 1)return 1500 -
                              difference;                                 //If the channel is reversed
    else return 1500 + difference;                                             //If the channel is not reversed
  }
  else return 1500;
}
