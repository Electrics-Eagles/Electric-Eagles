...
void gps_setup(void) {
  ...
  gps_serial.write(Disable_GPGSV, 11);
  ...
  gps_serial.write(Set_to_5Hz, 14);
  delay(350);   //A small delay is added to give the GPS some time to respond @ 9600bps.
  ...
  gps_serial.write(Set_to_57kbps, 28);
  delay(200);
...
void read_gps(void) {
  while (gps_serial.available() && new_line_found == 0) {                                                   //Stay in this loop as long as there is serial information from the GPS available.
    char read_serial_byte = gps_serial.read();                                                              //Load a new serial byte in the read_serial_byte variable.
  ...
  if (new_line_found == 1) {                                                                             //If a new NMEA line is found.
    ...
    if (incomming_message[4] == 'L' && incomming_message[5] == 'L' && incomming_message[7] == ',') {     //When there is no GPS fix or latitude/longitude information available.
      tone(BUZZER_PIN, 525);                                    //Change the LED on the STM32 to indicate GPS reception.
  ...
  if (new_gps_data_available) {                                                                           //If there is a new set of GPS data available.
    if (number_used_sats < 8) make_gps_ok_sound();                //Change the LED on the STM32 to indicate GPS reception.
      else  make_gps_ok_sound();                                                              //Turn the LED on the STM solid on (LED function is inverted). Check the STM32 schematic.
        gps_watchdog_timer = millis();                                                                        //Reset the GPS watch dog tmer.
        new_gps_data_available = 0;                                                                           //Reset the new_gps_data_available variable.

        if ( waypoint_set == 0) {                                                          //If the flight mode is 3 (GPS hold) and no waypoints are set.
  ...
  if ( waypoint_set == 1) {                                                          //If the GPS hold mode and the waypoints are stored.
    ...
    gps_lat_total_avarage -=  gps_lat_rotating_mem[ gps_rotating_mem_location];                         //Subtract the current memory position to make room for the new value.
      ...
      if (GPS_ON == 1) {
        ...
        gps_roll_adjust = ((float)gps_roll_adjust_north * cos(sin(gyro_yaw * 0.000001066) * 0.017453)) + ((float)gps_pitch_adjust_north * cos((sin(gyro_yaw * 0.000001066) - 90) * 0.017453));
        gps_pitch_adjust = ((float)gps_pitch_adjust_north * cos(sin(gyro_yaw * 0.000001066) * 0.017453)) + ((float)gps_roll_adjust_north * cos((sin(gyro_yaw * 0.000001066) + 90) * 0.017453));
      } else {
        gps_roll_adjust = 0;
        gps_pitch_adjust = 0;
      }
